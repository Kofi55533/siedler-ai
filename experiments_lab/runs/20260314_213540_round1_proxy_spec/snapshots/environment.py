...
class SiedlerScharfschuetzenEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self._last_scharf_required_buildings_completed = (
            self._get_scharf_required_buildings_completed_count()
        )
        self._last_scharf_required_techs_completed = (
            self._get_scharf_required_techs_completed_count()
        )
    ...
    def _get_scharf_required_buildings_completed_count(self) -> int:
        required_buildings = self._scharf_required_buildings or set()
        if not required_buildings:
            return 0
        return int(
    ...
    def _get_scharf_required_techs_completed_count(self) -> int:
        required_techs = self._scharf_required_techs or set()
        if not required_techs:
            return 0
    ...
    def _mask_position_index(self):
        ...
        if building:
            if self.current_flow == "assign_serf":
                # _get_build_position_candidates nutzen: korrekte Mine/DZ-Slots
                total_count = len(self._get_build_position_candidates(building))
            else:
    ...
    def _tick_time(self):
        ...
        # RegularTax = fester Betrag PRO WORKER (nicht Multiplikator!)
        if (self._first_worker_building_time is not None
                and self.current_time > self._first_worker_building_time
                and (self.current_time - self._first_worker_building_time) % INCOME_CYCLE == 0):
            tax_info = TAX_LEVELS.get(self.current_tax_level, TAX_LEVELS[2])
